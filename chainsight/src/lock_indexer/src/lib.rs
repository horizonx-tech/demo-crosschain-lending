...
#[query]
#[candid_method(query)]
fn events_latest_n(size: usize) -> Vec<LockCreatedEvent> {
...
fn finder() -> LogFinder {
    ...
    .unwrap();
    LogFinder::new(web3, contract, "LockCreated")
}
...
async fn save_logs_from_to(from: u64, to: u64) -> Result<(), ()> {
    let finder = finder();
    let events: HashMap<u64, Vec<LockCreatedEvent>> = finder
        .find(from, to)
        .await
        .unwrap()
        .into_iter()
        .map(|log| LockCreatedEvent::from(log, NETWORK.with(|nw| nw.borrow().to_owned())))
    ...
    let data: Vec<LockCreatedEvent> = events.values().flatten().cloned().collect();
