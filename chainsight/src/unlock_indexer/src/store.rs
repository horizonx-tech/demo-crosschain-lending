...
type Events = SBTreeMap<SBox<u64>, SVec<SBox<LockReleasedEvent>>>;
...
pub fn events_latest_n(n: usize) -> Vec<LockReleasedEvent> {
    let mut events: Vec<LockReleasedEvent> = Vec::new();
    STATE.with(|f| {
        let mut count: usize = 0;
...
pub fn add_events(block_number: u64, events: Vec<LockReleasedEvent>) {
    STATE.with(|f| {
        let mut state = f.borrow_mut();
        let mut events = events;
        let mut events_boxed: SVec<SBox<LockReleasedEvent>> = SVec::new();
